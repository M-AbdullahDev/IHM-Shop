...
const Ledger = {
    ...
    handleAuth() {
        ...
        const storedPassword = localStorage.getItem('zyro_admin_password') || 'admin';

        if (pwdInput.value === storedPassword) {
            ...
            errorMsg.textContent = 'Verification failed: Password incorrect.';
    ...
    render(filter = '') {
        ...
        let items = isClothing ? Store.getInventory() : Store.getAccessories();
        ...
        if (totalValEl) totalValEl.textContent = UI.formatCurrency(totalInventoryValue);
        ...
        if (groupedList.length === 0) {
            container.innerHTML = `<tr><td colspan="7" style="text-align: center; color: var(--text-muted); padding: 3rem;">No ${this.currentTab} items found.</td></tr>`;
            return;
        }
